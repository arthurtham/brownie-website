...
if (isset($_GET["blog_id"])) {
    ...
    // Check if old ID or new ID. All old IDs are numbers that are 5 digits long. New IDs are UUIDs.
    $is_new_id = is_new_blog_id($_GET["blog_id"]);
    if ($is_new_id) {
        $sql = "SELECT * FROM blog_posts WHERE blog_new_id = UUID_TO_BIN(\"". mysqli_real_escape_string($conn, $_GET["blog_id"])."\") LIMIT 1;";
    } else {
        $sql = "SELECT * FROM blog_posts WHERE blog_id = \"". mysqli_real_escape_string($conn, $_GET["blog_id"])."\" LIMIT 1;";
    }
...
BLOGURL : "";
    ...
    //     }
    // }
    $blog_id = -1; // New ID system will be used, so this is just a placeholder and will be ignored. The new blog ID will be auto-assigned by the database, and the old blog id will be nullified which is handled by the database.
    $blog_new_id = "-1"; // Placeholder until new ID is generated by database.
